...
void UpdateLeaderboardUI(List<PlayerLeaderboardEntry> leaderboard)
{
    for (int x = 0; x < leaderboardEntries.Length; x++)
    {
        ...
        leaderboardEntries[x].transform.Find("ScoreText").GetComponent<TextMeshProUGUI>().text =  "$" + (-(float)leaderboard[x].StatValue * 0.001).ToString("F2");
    }
}
    public void SetLeaderboardEntry(int newScore)
    {
        ...
        error =>
        {
            ...
            PlayFabClientAPI.UpdatePlayerStatistics(new UpdatePlayerStatisticsRequest
            {
                ...
                {
                    new StatisticUpdate { StatisticName = "Money", Value = newScore },
                }
            },
